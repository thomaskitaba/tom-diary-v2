...
def searchdates():
  ...
  # global_search_end_date[0] = str(end_date)
    ...
    if search_task_to_do[0] == "start-to-end-date": # or "from-begning-to-end-date":
      ...
      return render_template("search.html",current_user_name=global_user_name(), catagories=catagories, start_date= session["start_date"] , end_date= session["end_date"], catagory_types= catagory_types,all_catagory = user_catagories(), normal_search= 1, results = results)
...
def simpledate():
  ...
  if request.method == "POST":
    ...
    catagory_type_id = request.form.get("catagory_type")
    ...
    if date_upto == "upto":
      ...
      if not start_date and not end_date:
        ...
        if search_by_catagory_type == "search-by-type":
          
          search_task_to_do_additional[0] ="add-catagory-type"
          session["search-catagory_type"] = catagory_type_id
          
        elif sub_catagory_ids:
