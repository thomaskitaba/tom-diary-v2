...
app.config['SESSION_COOKIE_NAME'] = 'new_session_cookie_name'
...
def sampleindex():
    if request.method == "POST":
        msg = Message('tom-diary', sender='thomas.kitaba@gmail.com',
                      recipients=['thomas.kitaba@gmail.com'])
...
def user_profile_information():
    ...
    user_profile_information = db.execute(
        "SELECT * FROM users WHERE username = ?", name[0]["username"])

    return user_profile_information


def global_catagories():
    catagories = db.execute(
        "SELECT * FROM catagory JOIN catagorytype ON catagory.cat_type_id = catagorytype.catagory_type_id")
    return catagories


def global_catagory_with_parameter(cattype):
    catagories = db.execute(
        "SELECT * FROM catagory JOIN catagorytype ON catagory.cat_type_id = catagorytype.catagory_type_id WHERE catagory_type_id = ?", cattype)
...
def user_catagories():
    catagories = db.execute(
        "SELECT * FROM catagorytype JOIN usercatagory ON usercatagory.cat_type_id_in_uc = catagorytype.catagory_type_id JOIN users ON users.id = usercatagory.u_id_in_uc WHERE users.id = ? ", session["user_id"])


def default_user_catagories():
    catagories = db.execute(
        "SELECT * FROM catagory JOIN catagorytype ON catagory.cat_type_id = catagorytype.catagory_type_id JOIN usercatagory ON catagorytype.catagory_type_id = usercatagory.cat_type_id_in_uc JOIN users ON users.id = usercatagory.u_id_in_uc WHERE users.id = ? AND catagorytype.catagory_type_id = ?", session["user_id"], 1)

    return catagories


def default_all_user_catagories():
    all_user_catagories = db.execute(
        "SELECT * FROM catagory JOIN catagorytype ON catagory.cat_type_id = catagorytype.catagory_type_id JOIN usercatagory ON catagorytype.catagory_type_id = usercatagory.cat_type_id_in_uc JOIN users ON users.id = usercatagory.u_id_in_uc WHERE users.id = ?", session["user_id"])
...
def catagory_types():
    catagory_types = db.execute(
        "SELECT * FROM catagorytype JOIN usercatagory ON catagorytype.catagory_type_id = usercatagory.cat_type_id_in_uc JOIN users ON users.id= usercatagory.u_id_in_uc WHERE usercatagory.u_id_in_uc = ?", session["user_id"])
    return catagory_types
...
def insert_user_diary_catagory(userdiaryid, catagoryid):
    ...
    db.execute("INSERT INTO userdiarycatagory(ud_id, c_id, catagory_insertion_date, catagory_insertion_time  ) VALUES (?, ?, ?, ?)",
               userdiaryid, catagoryid, currentday(), currentclock())  # works


def get_udc_id():
    added_udc_id = db.execute(
        "SELECT * FROM userdiarycatagory ORDER BY udc_id DESC LIMIT 1")  # works
    return added_udc_id[0]["udc_id"]


def get_ud_id():
    added_ud_id = db.execute(
        "SELECT * FROM userdiary ORDER BY ud_id DESC LIMIT 1")  # works
    return added_ud_id[0]["ud_id"]


def get_d_id():
    added_d_id = db.execute(
        "SELECT * FROM diary ORDER BY diary_id DESC LIMIT 1")  # works
    return added_d_id[0]["diary_id"]


def get_diary_database_view():
    active = "Active"
    diary_database_view = db.execute(
        "SELECT * FROM diarydatabaseview WHERE diary_status = ?", "Active")
    return diary_database_view


def get_diary_content_info(user_diary_id):

    ud_id_info = db.execute("SELECT ud_id, u_id, trim(diary_content), given_date FROM diarydatabaseview WHERE u_id = ? AND ud_id = ? AND diary_status = ?",
                            session["user_id"], user_diary_id, 'Active')
    return ud_id_info


def get_diary_catagory_table(diaryid):
    diary_catagory_database_table = db.execute(
        "SELECT * FROM diary JOIN userdiary ON diary.diary_id = userdiary.d_id JOIN userdiarycatagory ON userdiarycatagory.ud_id = userdiary.ud_id JOIN catagory ON catagory.catagory_id = userdiarycatagory.c_id WHERE diary_id = ?", diaryid)
...
def get_catagory_name(userdiaryid):
    ...
    catagory = db.execute(
        "SELECT * FROM diarycatagoryview WHERE ud_id = ?", userdiaryid)
    return catagory
...
def get_diary_content(diaryid):
    ...
    diary = db.execute(
        "SELECT * FROM userdiaryview WHERE d_id = ? ORDER BY diary_written_date DESC", diaryid)
    return diary
# TODO: IMPORTANT ---------------------------------------------------------
...
def get_userdiary_reference(user_diary_id):
    ...
    user_diary_references = db.execute(
        "SELECT referencename.ref_type_id, referencetype.reference_type_name, referencename.reference_name_id, referencename.reference_name, diaryreference.referenced_by_ud_id,diaryreference.referenced_ud_id, diary.diary_content FROM referencetype JOIN referencename  ON referencetype.reference_type_id = referencename.ref_type_id JOIN diaryreference ON referencename.reference_name_id = diaryreference.ref_name_id JOIN userdiaryreference  ON diaryreference.reference_id = userdiaryreference.ref_id JOIN userdiary ON userdiaryreference.udr_id = userdiary.ud_id JOIN diary ON userdiary.ud_id = diary.diary_id WHERE userdiary.u_id = ? AND userdiaryreference.udr_id = ?", session["user_id"], user_diary_id)
...
def get_referenced_diary_content(referenced_userdiary_id):
    referenced_info = db.execute(
        "SELECT ud_id, diary_id, diary_content FROM userdiary JOIN diary ON userdiary.d_id = diary.diary_id WHERE userdiary.ud_id = ? ", referenced_userdiary_id)
...
def add_catagory_and_reference(list):
    ...
    for result in list:
        ...
        for referenced in result["content_reference"]:
            temp_ref_diary_content = get_referenced_diary_content(
                referenced["referenced_ud_id"])  # type: ignore
...
def get_reference_name():
    reference_name = db.execute(
        "SELECT * FROM referencename ORDER BY reference_name")
    return reference_name


def get_all_user_diary(userid):
    diary = db.execute(
        "SELECT * FROM userdiaryview WHERE id= ? and diary_status = ?", userid, "Active")
    return diary
...
def loadJsonReference():
    ...
    if rows[0]:
        for row in rows[0]:
            row["sub_reference"] = db.execute(
                "SELECT reference_name_id, reference_name FROM referencename WHERE ref_type_id = ?", row["reference_type_id"])
...
def get_all_user_diary_detail(userid):
    diary = db.execute(
        "SELECT  * FROM diarydatabaseview JOIN diarycatagoryview on diarydatabaseview.ud_id = diarycatagoryview.ud_id WHERE diarydatabaseview.id = ? and diarydatabaseview.diary_status=?", session["user_id"], "Active")
...
def generateJsonDiary():
    ...
    all_catagory_info = db.execute(
        "SELECT * FROM catagorytype JOIN usercatagory ON usercatagory.cat_type_id_in_uc = catagorytype.catagory_type_id JOIN users ON users.id = usercatagory.u_id_in_uc")
    # return render_template("experiment.html", cat_3= all_catagory_info )

    all_catagories = db.execute(
        "SELECT * FROM catagory JOIN catagorytype ON catagory.cat_type_id = catagorytype.catagory_type_id")
    # todo:
    ...
    for cat in all_catagory_info:
        ...
        for sub_cat in all_catagories:
            ...
            if sub_cat["cat_type_id"] == cat["catagory_type_id"]:
                ...
                temp_sub = dict(
                    {"catagory_id": sub_cat["catagory_id"], "catagory_name": sub_cat["catagory_name"]})
    ...
    all_catagory_info = db.execute(
        "SELECT username, catagory_type_name FROM userCatagoryInfo WHERE id = ? ", session["user_id"])
...
def register():
    ...
    if request.method == "POST":
        ...
        if len(firstname) > 0 or len(lastname) > 0 or len(birthday) > 0 or len(useremail) > 0 or len(password) >= 6 or len(confirmation) > 0 or (country):  # type: ignore
            ...
            rows = db.execute("INSERT INTO users (username, hash, fname, lname, useremail, dateofbirth, dateregistered, gender, primaryphone, secondaryphone, country, city, useraddress, facebookaddress, telegramaddress,instagramaddress, twitteraddress, emailconfirmed ) VALUES (?, ? , ? , ? , ? , ?, ?, ?, ? , ? , ? , ? , ?, ?, ?, ?, ?, ?)",
                              username, generate_password_hash(password), firstname, lastname, useremail, birthday, datetime.datetime.now(), gender, primary_phone, seconday_phone, country, city, user_address, facebook_address, telegram_address, instagram_address, twitter_address, "False")  # type: ignore
            # get the user id
            ...
            msg = Message('Dear' + ':' + str(firstname) + '  ' + str(lastname) +
                          '/n', sender='thomas.kitaba@gmail.com', recipients=[useremail])

            link = url_for('confirmUserEmail', token=token,
                           _external=True)    # import url_for from flask
...
def confirmUserEmail(token):
    ...
    try:
        username = serializer.loads(
            token, salt="email-confirmation", max_age=3600)  # expires after 1 hour
        # change users.emailconfirmd = True
        # rows = db.execute("SELECT * FROM users WHERE id = ?", session["user_id"])

        db.execute(
            "UPDATE users SET emailconfirmed = ? WHERE username = ?", "True", username)
...
def catagory():
    ...
    if request.method == "POST":
        ...
        rows = db.execute(
            "SELECT * FROM catagory WHERE catagory_name = ?", str(catagoryname))
        ...
        if len(catagoryname) != 0 and not rows:
            ...
            rows = db.execute("INSERT INTO catagory(cat_type_id, catagory_name, insertion_date, insertion_time) VALUES(?, ?, ?, ?)",
                              catagory_type, catagoryname, currentday(), currentclock())
...
def insertcatagorytype():
    ...
    if request.method == "POST":
        ...
        if catagory_type_name:
            ...
            catagory_type = db.execute(
                "SELECT * FROM catagorytype WHERE  catagory_type_name= ?", catagory_type_name)
            if not catagory_type:
                db.execute("INSERT INTO catagorytype (catagory_type_name, catagory_type_creation_date, catagory_type_creation_time, type_description) VALUES (? , ?, ?, ?)",
                           catagory_type_name,  currentday(), currentclock(), type_description)
                # write success
...
# TODO: diary will be deleted here meaning the status of the diary will be changed to "Deleted"----------------------------
    ...
    if task_to_do[0] == "editdiarydate":
        ...
        userdiary = db.execute(
            "SELECT * FROM userdiary WHERE ud_id = ? ", global_user_diary_id[0])
        number_of_edits = userdiary[0]["number_of_edits"]

        # increment number of edites by one
        number_of_edits += 1
    # STEP 2 MAKE THE ACTUAL CHANGE
        db.execute("UPDATE userdiary SET number_of_edits = ?, given_date = ?, given_time = ? WHERE ud_id = ?",
                   number_of_edits, global_diary_date[0], global_diary_time[0], global_user_diary_id[0])
...
# TODO: diary will be deleted here meaning the status of the diary will be changed to "Deleted"----------------------------
    ...
    if task_to_do[0] == "deletediary":
        ...
        db.execute("UPDATE diary SET diary_status = ? WHERE diary_id = ?",
                   "Deleted", global_diary_id[0])
        # db.execute("COMMIT")
    ...
    if task_to_do[0] == "editdiarycontent":
        ...
        db.execute("UPDATE diary SET diary_content = ?  WHERE diary_id = ?",
                   global_diary_content[0], global_diary_id[0])

        # increment number of edits to this specific diary by one then update
        # increment
        global_diary_number_of_edits[0] = int(
            global_diary_number_of_edits[0]) + 1  # type: ignore
        # update
        db.execute("UPDATE userdiary SET number_of_edits = ? WHERE ud_id = ?",
                   global_diary_number_of_edits[0], global_user_diary_id[0])
...
def deletediary():
    ...
    if request.method == "POST":
        ...
        global_diary_element_id[0] = request.form.get(
            "diary-element-id")  # type: ignore
...
def editdiarycontent():
    ...
    if request.method == "POST":
        ...
        global_diary_content[0] = request.form.get(
            "diary-content")  # type: ignore
        global_diary_number_of_edits[0] = request.form.get(
            "number-of-edits")  # type: ignore
        global_diary_id[0] = request.form.get("diary-id")   # type: ignore
        global_user_diary_id[0] = request.form.get(
            "user-diary-id")  # type: ignore
        global_diary_element_id[0] = request.form.get(
            "diary-element-id")  # type: ignore
...
def generatecatagory():
    ...
    if request.method == "POST":
        ...
        catagory_type = db.execute(
            "SELECT * FROM catagorytype WHERE catagory_type_id =? ", catagory_type_id)
        ...
        catagory_types.insert(
            0, {"catagory_type_id": catagory_type_id, "catagory_type_name": catagory_type_name})
...
def jsonajax():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO diary (diary_content, description ) VALUES (?, ?)",
                   diary_text, diary_description)  # works

        # step 2  get the inserted log/diary id

        new_diary_id = db.execute(
            "SELECT * FROM diary ORDER BY diary_id DESC LIMIT 1")  # works
        ...
        db.execute("INSERT INTO userdiary (d_id, u_id, given_date, given_time, diary_written_date, diary_written_time, start_date, end_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
                   new_diary_id[0]["diary_id"], session["user_id"], given_date, given_time, currentday(), currentclock(), start_date, end_date)

        # session["added_user_diary_id"] = db.execute("SELECT * FROM userdiary ORDER BY ud_id DESC LIMIT 1")
        rows = db.execute(
            "SELECT * FROM userdiary ORDER BY ud_id DESC LIMIT 1")
        ...
        added_user_diary_id = db.execute(
            "SELECT * FROM userdiary ORDER BY ud_id DESC LIMIT 1")  # works
        ...
        # find id of Normal
        default = "Normal"
        catagory_normal = db.execute(
            "SELECT * FROM catagory WHERE catagory_name = ?", default)  # works
        catagory_normal_id = int(catagory_normal[0]["catagory_id"])  # works

        if not multiple_catagories_selected:
            catagory_normal_id = int(
                catagory_normal[0]["catagory_id"])  # works
            db.execute("INSERT INTO userdiarycatagory (ud_id, c_id, catagory_insertion_date, catagory_insertion_time ) VALUES (?, ?, ?, ? )",
                       session["added_user_diary_id"], catagory_normal_id, currentday(), currentclock())
            return redirect("/viewdiary")
        if multiple_catagories_selected[0] != "":
            ...
            for i in range(len(splited_multiple_catagories)):
                ...
                db.execute("INSERT INTO userdiarycatagory (ud_id, c_id, catagory_insertion_date, catagory_insertion_time ) VALUES (?, ?, ?, ? )", int(
                    session["added_user_diary_id"]), int(splited_multiple_catagories[i]), currentday(), currentclock())
...
def viewtodo():
    ...
    if request.method == "POST":
        ...
        todolist = db.execute("SELECT * FROM diary JOIN userdiary ON diary.diary_id = userdiary.d_id JOIN userdiarycatagory ON userdiarycatagory.ud_id = userdiary.ud_id JOIN catagory ON catagory.catagory_id = userdiarycatagory.c_id JOIN catagorytype ON catagorytype.catagory_type_id =catagory.cat_type_id WHERE u_id = ? and catagory_type_name = ? and diary_status = ?",
                              session["user_id"], "My Todo List", "Active")
...
def diarywrite():
    ...
    if request.method == "POST":
        ...
        # step 3   update user diary table with diaryid and and user id
        db.execute("INSERT INTO userdiary (d_id, u_id, given_date, given_time, diary_written_date, diary_written_time) VALUES (?, ?, ?, ?, ?, ?)",
                   new_diary_id[0]["diary_id"], session["user_id"], given_date, given_time, currentday(), currentclock())
...
def diaryview():
    ...
    for diary_info in user_diary:
        ...
        for count, catagory_info in enumerate(catagory_list):
            ...
            diary_catagories.append(
                [catagory_info["catagory_id"], catagory_info["catagory_name"], catagory_info["udc_id"]])
    ...
    diary_count = db.execute(
        "SELECT * FROM userdiary WHERE u_id = ?", session["user_id"])
    diary_count = len(diary_count)

    edited_diary_element_id = global_edited_diary[0]

    if edited_diary_element_id != diary_count:
        edited_diary_element_id = int(
            global_edited_diary[0]) + 1  # type: ignore
...
# --------------------------- works to view all data with duplicate ------------------------------
    diary = db.execute(
        "SELECT * FROM diarydatabaseview WHERE id = ?", session["user_id"])
...
# ------------------------ WORKS ------------------------------------------------------------------------------
    ...
    #diary = db.execute("SELECT * FROM userdiaryview WHERE d_id = ?", did)
        catagory = db.execute(
            "SELECT * FROM diarycatagoryview WHERE ud_id = ?", udid)
...
def addDiaryReference():
    ...
    if request.method == "POST":  # type: ignore
        ...
        for i in range(len(converted_referenced_id)):
            ...
            db.execute("INSERT INTO diaryreference (referenced_by_ud_id, referenced_ud_id, ref_name_id, diary_ref_insertion_date, diary_ref_insertion_time) VALUES (?, ?, ?, ?, ?)",
                       referenced_by_id, converted_referenced_id[i], reference_name_id[0], currentday(), currentclock())

            # TODO: find ref_id of last inserted data in diaryreference
            index = db.execute(
                "SELECT * FROM diaryreference ORDER BY reference_id DESC LIMIT 1")
            ...
            db.execute("INSERT INTO userdiaryreference (ref_id, udr_id) VALUES (?, ?)",
                       last_inserted_id[0], referenced_by_id)

        # TODO: redisplay search results with all user diary data
        rows = db.execute(
            "SELECT * FROM userdiary WHERE u_id = ? lIMIT 1 ", session["user_id"])
...
def changecatagory():
    if request.method == "POST":
        ...
        rows = db.execute(
            "SELECT * FROM userdiarycatagory WHERE udc_id = ?", user_diary_catagory_id)
        number_of_edits = int(rows[0]["number_of_edits_made"]) + 1

        #user_diary_catagory_id = request.form.get("user-diary-catagory-id")
        db.execute("UPDATE userdiarycatagory SET c_id = ?, number_of_edits_made = ?,catagory_insertion_date = ?, catagory_insertion_time = ? WHERE udc_id = ?",
                   catagory_id, number_of_edits, currentday(), currentclock(), user_diary_catagory_id)
...
def removecatagory():
    ...
    if request.method == "POST":
        ...
        rows = db.execute(
            "SELECT * FROM userdiary WHERE ud_id = ?", user_diary_id)
        # increment it by one
        number_of_edits = int(rows[0]["number_of_edits"]) + 1

        # update edit field
        db.execute("UPDATE userdiary SET number_of_edits = ? WHERE ud_id = ? ",
                   number_of_edits, user_diary_id)

        # this is the main UPDATE (update removed data to userdiary catagory )
        # TODO: send udc_id   to be used as ID  - inorder to take user back to the editeded row

        db.execute("DELETE FROM userdiarycatagory WHERE udc_id = ?",
                   user_diary_catagory_id)
...
# return render_template("diary.html", edit_diary = 1, us)
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO userdiarycatagory (ud_id, c_id, catagory_insertion_date, catagory_insertion_time) VALUES(?, ?, ?, ? )",
                   ud_id, catagory_tobe_added_id, currentday(), currentclock())
...
# type: ignore
@app.route("/advancedsearchcatagories", methods=["GET", "POST"])
@login_required
def advancedsearchcatagories():
    if request.method == "POST":
        ...
        catagory_types = db.execute(
            "SELECT * FROM catagorytype JOIN usercatagory ON catagorytype.catagory_type_id = usercatagory.cat_type_id_in_uc JOIN users ON users.id = usercatagory.u_id_in_uc WHERE users.id = ?", session["user_id"])
...
def diarysearch():
    ...
    if request.method == "POST":
        ...
        if not search_catagory_id:
            results = [db.execute(
                "SELECT DISTINCT * FROM diarydatabaseview WHERE id= ? AND diary_status = ? ORDER BY given_date DESC", session["user_id"], 'Active')]
            ...
            results = [db.execute("SELECT DISTINCT * FROM diarydatabaseview WHERE c_id= ? and u_id = ? and diary_status = ? ORDER BY given_date DESC",
                                  search_catagory_id, session["user_id"], 'Active')]
        ...
        rows = db.execute(
            "SELECT catagory_name FROM catagory WHERE catagory_id = ?", search_catagory_id)
        if rows:
...
def searchdates():
    ...
    # todo: or "from-begning-  to  -end-date":
    if search_task_to_do[0] == "start-to-end-date":
        ...
        if search_task_to_do_additional[0] == "add-catagory-type":
            ...
            results = [db.execute("SELECT DISTINCT * FROM diarydatabaseview WHERE cat_type_id = ? AND given_date >= ? AND given_date <= ? GROUP BY ud_id ORDER BY given_date DESC",
                                  session["catagory_type_id"], session["start_date"], session["end_date"])]
            # return render_template("experiment.html", cat_3 = results[0])
            number_of_results = len(results[0])
        ...
        if search_task_to_do_additional[0] == "only-date-upto":
            ...
            results[0] = db.execute(
                "SELECT DISTINCT * FROM diarydatabaseview WHERE given_date >= ? AND given_date <= ? GROUP BY ud_id ORDER BY given_date DESC", session["start_date"], session["end_date"])
        ...
        if search_task_to_do_additional[0] == "add-sub-catagories":
            ...
            for i in range(len(sub_catagory)):
                ...
                rows = db.execute("SELECT DISTINCT * FROM diarydatabaseview WHERE catagory_id = ? AND id = ? AND given_date >= ? AND given_date <= ? GROUP BY ud_id ORDER BY given_date DESC",
                                  int(sub_catagory[i]), session["user_id"], session["start_date"], session["end_date"])
                if rows:
                    for row in rows:
                        results.append(row)
    ...
    if search_task_to_do[0] == "specific-date":
        ...
        if search_task_to_do_additional[0] == "add-catagory-type":
            ...
            results = db.execute("SELECT DISTINCT * FROM diarydatabaseview WHERE cat_type_id = ? AND id = ? AND given_date Like ? AND diary_status= ? ORDER BY given_date DESC",
                                 session["catagory_type_id"], session["user_id"], "%" + global_search_start_date[0] + "%",  "Active")
        ...
        if search_task_to_do_additional[0] == "add-sub-catagories":
            ...
            for i in range(len(sub_catagory)):
                ...
                rows = db.execute("SELECT DISTINCT * FROM diarydatabaseview WHERE catagory_id = ? AND id = ? AND given_date Like ? AND diary_status= ? ORDER BY given_date DESC",
                                  int(sub_catagory[i]), session["user_id"], "%" + global_search_start_date[0] + "%",  "Active")
                if rows:
                    for row in rows:
                        results.append(row)
        ...
        if search_task_to_do_additional[0] == "all-sub-catagories":
            # return render_template("experiment.html", cat_3 = "search all sub catagories")
            results.clear()
            results = db.execute("SELECT DISTINCT * FROM diary JOIN userdiary ON diary.diary_id = userdiary.d_id WHERE u_id = ? AND  userdiary.given_date Like ? AND diary_status = ? ORDER BY userdiary.given_date DESC",
                                 session["user_id"], "%" + global_search_start_date[0] + "%",  "Active")
...
def simpledate():
    ...
    if request.method == "POST":
        ...
        search_by_catagory_type = str(
            request.form.get("search-type-check-box"))
...
def editprofile():
    if request.method == "POST":
        ...
        rows = db.execute(
            "SELECT * FROM users WHERE id LIKE ?", session["user_id"])
        ...
        if old_password:
            ...
            if not check_password_format(new_password):
                flash(
                    "Password shold be more than 5 character long and should contain at least 1 letter")
                return redirect("/editprofile")
            ...
            updated_profile = db.execute("UPDATE users SET useremail = ?, hash = ?, facebookaddress = ?,  telegramaddress = ?, instagramaddress = ?, twitteraddress = ?, country = ?, city = ?, gender = ?, primaryphone = ?, secondaryphone = ? , useraddress = ?, dateofbirth = ?, numberofprofilechanges = ?",
                                         user_email, generate_password_hash(str(new_password)), facebook_address, telegram_address, instagram_address, twitter_address, country, city, gender, primary_phone, secondary_phone, user_address, dateofbirth, number_of_changes)

            # TODO: send email message telling user that he has successfully changed his profile
            msg = Message('Dear' + ':' + upi["fname"] + '  ' + upi["lname"] +
                          '/n', sender='thomas.kitaba@gmail.com', recipients=[user_email])
            msg.body = "your profile has been updated:  number of profile edits including today =" + \
        ...
        #   return redirect("/editprofile")
            ...
            db.execute("UPDATE users SET useremail = ?, facebookaddress = ?,  telegramaddress = ?, instagramaddress = ?, twitteraddress = ?, country = ?, city = ?, gender = ?, primaryphone = ?, secondaryphone = ? , useraddress = ?, dateofbirth = ?, numberofprofilechanges = ?",
                       user_email, facebook_address, telegram_address, instagram_address, twitter_address, country, city, gender, primary_phone, secondary_phone, user_address, dateofbirth, number_of_changes)
...
def universaldiary():
    ...
    # type: ignore
    return render_template("experiment.html", current_user_name=global_user_name(), cat_3=rows[0])
...
def references():
    ...
    # type: ignore
    return render_template("experiment.html", current_user_name=global_user_name(), cat_3=temp)
...
def stripdiary():
    if request.method == "post":
        ...
        # type: ignore
        return render_template("experiment.html", current_user_name=global_user_name(), cat_3=get_userdiaries_catagories())
        ...
        for row in rows:
            ...
            db.execute("UPDATE diary SET diary_content = ? WHERE diary_id = ?",
                       new_diary_content, row["diary_id"])
